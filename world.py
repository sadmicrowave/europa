...
class World:
	...
	def construct_room(self):
		...
		for index,row in enumerate( sheet.iter_rows() ):
			...
			if index > 0: 
				...
				isBlocked	  = row[5].value if row[5].value else ''
	...
	def load_gold(self,name='gold'):
		"""Parse the world spreadsheet Gold tab containing object data for the passed spreadsheet name's items"""
		# open the workbook containing our world contents
		wb = load_workbook( filename = self.world_file )
		# select the specific worksheet to load data from
		sheet = wb.get_sheet_by_name(name.capitalize())
	...
	def load_purse(self):
		...
		self._objects['Purse'] = items.Purse()
